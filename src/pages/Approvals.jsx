...
export default function Approvals() {
    ...
    const loadReports = async () => {
        ...
        try {
            ...
            // ✅ IMPROVED: Sort by createdAt (latest first)
        ...
        <div style={{ padding: '32px 40px', maxWidth: 1400, display: 'grid', gridTemplateColumns: selected ? '380px 1fr' : '1fr', gap: 24 }}>
            ...
            <div>
                ...
                <div className="fade-up" style={{ marginBottom: 20 }}>
                    ...
                    <h1 style={{ margin: '6px 0 0', fontFamily: 'var(--font-display)', fontSize: 42, color: 'var(--text-white)' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }}>
                    <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, padding: 12 }}>
                        ...
                        <p style={{ margin: '4px 0 0', fontSize: 10, color: 'var(--text-white)', letterSpacing: '0.5px' }}>PENDING REVIEW</p>
                    </div>
                    <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, padding: 12 }}>
                        <p style={{ margin: 0, fontSize: 24, color: 'var(--accent)', fontFamily: 'var(--font-display)', fontWeight: 700 }}>{awaitingCount}</p>
                        <p style={{ margin: '4px 0 0', fontSize: 10, color: 'var(--text-white)', letterSpacing: '0.5px' }}>AWAITING APPROVAL</p>
                    </div>
                </div>

                {/* Loading State */}
                {loading ? (
                    <div style={{ textAlign: 'center', padding: '40px 20px', color: 'var(--text-white)' }}>
                    ...
                    <div style={{ textAlign: 'center', padding: '40px 20px', color: 'var(--text-white)', fontSize: 13 }}>
                        ✅ Sabhi reports approved!
                    </div>
                ) : (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10, maxHeight: '70vh', overflowY: 'auto' }}>
                        {reports.map(r => (
                            <div
                                ...
                                style={{
                                    padding: 12,
                                    background: selected?._id === r._id ? 'var(--bg-elevated)' : 'var(--bg-card)',
                                    border: selected?._id === r._id ? '1px solid var(--accent)' : '1px solid var(--border)',
                                    borderRadius: 8,
                                    cursor: 'pointer',
                                    transition: 'all 0.2s'
                                }}
                            >
                                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 8 }}>
                                    <div style={{ flex: 1 }}>
                                        <p style={{ margin: '0 0 2px', fontSize: 13, fontWeight: 700, color: 'var(--text-white)' }}>
                                            {getClientName(r)}
                                        </p>
                                        <p style={{ margin: 0, fontSize: 11, color: 'var(--text-white)', opacity: 0.7 }}>
                                    ...
                                    <div
                                        style={{
                                            background: STATUS_COLORS[r.approvalStatus].bg,
                                            color: STATUS_COLORS[r.approvalStatus].color,
                                            padding: '2px 8px',
                                            borderRadius: 4,
                                            fontSize: 9,
                                            fontWeight: 700,
                                    ...
                                    >
                                        {STATUS_COLORS[r.approvalStatus].label}
                                    </div>
                                </div>
                                <p style={{ margin: 0, fontSize: 11, color: 'var(--text-white)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                                    {r.reportType}
                                </p>
                            </div>
                        ))}
            ...
            {/* RIGHT — Preview Panel */}
            {selected && (
                <div style={{ display: 'flex', flexDirection: 'column', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' }}>
                    ...
                    {previewLoading ? (
                        ...
                        <>
                            {/* ✅ NEW: Document Info Header with Metadata */}
                            <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border)', flexShrink: 0, background: 'var(--bg-elevated)' }}>
                                <p style={{ margin: '0 0 8px', fontSize: 10, color: 'var(--text-white)', letterSpacing: '0.5px', opacity: 0.8 }}>DOCUMENT INFO</p>
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                    <div>
                                        <p style={{ margin: 0, fontSize: 14, fontWeight: 700, color: 'var(--text-white)' }}>
                                            📄 {preview.clientName}
                                        </p>
                                        <p style={{ margin: '4px 0 0', fontSize: 11, color: 'var(--text-white)', opacity: 0.7 }}>
                                            {new Date(preview.startDate).toLocaleDateString('en-IN')} → {new Date(preview.endDate).toLocaleDateString('en-IN')}
                                            {' • '} {preview.reportType?.toUpperCase()}
                                        </p>
                                    </div>
                                    <div
                                        style={{
                                            background: STATUS_COLORS[preview.approvalStatus].bg,
                                            color: STATUS_COLORS[preview.approvalStatus].color,
                                            padding: '4px 12px',
                                            borderRadius: 6,
                                            fontSize: 10,
                                            fontWeight: 700,
                                    ...
                                    >
                                        {STATUS_COLORS[preview.approvalStatus].label}
                                    </div>
                                </div>
                            </div>
                                ...
                                <div
                                    ...
                                    dangerouslySetInnerHTML={{
                                        __html: showAI
                                            ...
                                            : (preview.rawDocContent || '<p style="color: orange;">⚠️ Original document content nahi mila. Report create karte time document content pass karna zaroori hai.</p>')
                                    }}
                                />
                            </div>
