...
export default function Reports() {
  ...
  // ✅ IMPROVED: Handle document content paste/upload
  const handleDocContentChange = (e) => {
    const content = e.target.value;
    setForm(prev => ({ ...prev, rawDocContent: content }));
  };
  ...
  return (
    <div style={{ padding: '32px 40px', maxWidth: 1300 }}>
      ...
      <div className="fade-up" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 24 }}>
        ...
        <button
          onClick={() => { setModal(true); setForm(EMPTY_FORM); setFormError(''); }}
          style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'var(--accent)', border: 'none', borderRadius: 8, color: '#0a0b0e', fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 14, padding: '12px 20px', cursor: 'pointer' }}
        >
      ...
      {/* Error Alert */}
      {error && (
        <div style={{ margin: '0 0 20px', background: 'var(--warn-dim)', border: '1px solid var(--warn)', borderRadius: 8, padding: '12px 16px', color: 'var(--warn)', fontSize: 13 }}>
          ⚠️ {error}
        </div>
      )}
      ...
      <div style={{ marginBottom: 24 }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12 }}>
          ...
          {hasFilters && (
            <button onClick={clearFilters} style={{ background: 'var(--border)', border: 'none', borderRadius: 8, color: 'var(--text-white)', fontFamily: 'var(--font-mono)', fontSize: 12, padding: '10px', cursor: 'pointer' }}>
              Clear Filters
            </button>
          )}
        </div>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
                      {r.pdfPath && (
                        <button
                          ...
                          style={{ background: 'transparent', border: '1px solid var(--border)', borderRadius: 6, color: 'var(--accent)', padding: '6px 12px', cursor: 'pointer', fontSize: 12 }}
                        >
                          <Download size={14} />
                        </button>
                      )}
                      <button
                        ...
                        style={{ background: 'transparent', border: '1px solid var(--border)', borderRadius: 6, color: 'var(--warn)', padding: '6px 12px', cursor: 'pointer', fontSize: 12 }}
                      >
                        <Trash2 size={14} />
                      </button>
                    </div>
          ...
          <div style={{ minWidth: 480 }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button
                ...
                style={{ flex: 1, background: 'transparent', border: '1px solid var(--border)', borderRadius: 8, color: 'var(--text-white)', fontFamily: 'var(--font-mono)', fontSize: 13, padding: '10px', cursor: 'pointer' }}
              >
                Cancel
              </button>
              <button
      ...
      {delModal && (
        <Modal onClose={() => setDelModal(null)}>
          <div style={{ minWidth: 360 }}>
            <h2 style={{ margin: '0 0 12px', fontFamily: 'var(--font-display)', fontSize: 18 }}>Delete Report?</h2>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setDelModal(null)} style={{ flex: 1, background: 'transparent', border: '1px solid var(--border)', borderRadius: 8, color: 'var(--text-white)', fontFamily: 'var(--font-mono)', fontSize: 13, padding: '9px', cursor: 'pointer' }}>Cancel</button>
              <button onClick={handleDelete} style={{ flex: 1, background: 'var(--warn)', border: 'none', borderRadius: 8, color: '#fff', fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 13, padding: '9px', cursor: 'pointer' }}>Delete</button>
            </div>
          </div>
        </Modal>
      )}
